## Multistar vs multistar2: core behavior differences

### Short answer
**No** — multistar2 inherits a lot of the *UI/features* from multistar, but it does **not** implement all of the same *behavioral safeguards* (especially around secondary-star “sanity checking” like the multistar zero-count / miss-count / sigma-based gating).

### What’s the same (user-facing “features”)
Because `GuiderMultiStar2` **inherits from** `GuiderMultiStar` and does **not** replace the click handler, you still get the multistar interaction model:
...
- **Multistar** has explicit secondary-star rejection logic for suspicious cases, e.g.:
...
So: **hot pixel rejection is not identical**. Some hot pixels may still get filtered by HFD/SNR logic, but multistar2 does not have the multistar “DZ/zeroCount” mechanism as written.

#### 2) “Only apply multistar if it helps” rule
- **Multistar** computes a refined (multi-star) offset, but then applies it only if it’s **smaller than the primary-only delta** (the `hypot(sumX,sumY) < primaryDistance` check).
...
- **Multistar** has a stabilization concept (enter/exit stabilization, update reference points after lock position moves, etc.) and uses statistics like `primarySigma`.
...
- If you mean **all the multistar robustness heuristics** (hot-pixel rejection mechanics, miss-count, sigma-based stabilization, “only apply if better”): **multistar2 differs**, and in some cases is missing those exact mechanisms (intentionally or not).

If you want, list the specific multistar behaviors you care most about (“DZ hot pixel removal”, “miss-count reset”, “stabilization period”, etc.) and I’ll map each one to: **present as-is / replaced by different mechanism / currently absent** in multistar2.
