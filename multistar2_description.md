...
This document describes **multistar2**, the experimental multi-star guider implementation added as an opt-in alternative to multistar (`GuiderMultiStar`).

Multistar2 is selected via Advanced Settings, and is intentionally implemented as a separate guider class so it can evolve without destabilizing multistar.

---

## Big-picture overview

Multistar2 keeps the same overall user intent (“guide using multiple stars”) but changes the *core tracking model* to address multistar’s main shortcomings:
...
Multistar2 still respects the existing “Use subframes” behavior: if the camera is operating with subframes/ROI, multi-star operation is effectively constrained (similar to multistar behavior).
...
Multistar refines the primary’s offset using secondary star displacements, but remains anchored to the primary. Multistar2 instead computes the guiding solution using:
...
- **Improvement vs multistar**: avoids subtle drift or steps caused by newly reacquired stars entering the averaging set.

### 5) Per-star mass-change rejection

Multistar mass-change handling is primary-centric. Multistar2 applies a mass-change check **per star**, and if a star’s mass appears inconsistent it is excluded from contributing (treated as a bad measurement).

- **Improvement vs multistar**: one “bad” star does not force a lost-star frame; it simply stops contributing.
...
- **Primary marker**: green box around the aggregate solution, with outward ticks (to visually distinguish multistar2 from multistar)
- **Status text**: bottom-right “Multistars: X/Y” where:
...
## Similarities to multistar and baseline guiders

- **Similar to multistar**:
...
## Summary of improvements vs multistar
